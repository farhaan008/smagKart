...
.controller('IniLoginCtrl', function($scope,$rootScope,$location,$ionicSideMenuDelegate,$ionicHistory, usersService) {
	'use strict';
	$ionicSideMenuDelegate.canDragContent(false); // hide sidemenu
	...
	$scope.userLogin = function(form){		
		if(form.$valid) {		
			$scope.iniLogin.token = "e6e86b5a0bebfcec3320ce97d39ebced";
			//usersService.userLogin($scope.iniLogin);			
			if($scope.iniLogin.email=="farhaaniit80@gmail.com" && $scope.iniLogin.password=="1234"){				
				$location.path("app/dashboard");
			}
			else{
